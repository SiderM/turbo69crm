  ...
  <tr class="flex w-full mb-2">
    ...
    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500 text-sm leading-5 w-1/5">
      <span class="relative inline-block px-3 py-1 font-semibold leading-tight">
        <span
          :class="{ 'bg-green-500 text-green-900': incom.status == 'inOrder', 'bg-indigo-500 text-indigo-900': incom.status == 'new', 'bg-blue-500 text-blue-900': incom.status == 'calling', 'bg-red-500 text-red-900': incom.status == 'break' }"
          class="absolute inset-0 opacity-50 rounded-full"></span>
        <span v-if="incom.status == 'inOrder'" class="relative text-xs">В заказах</span>
        <span v-else-if="incom.status == 'calling'" class="relative text-xs">Обзвонен</span>
        <span v-else-if="incom.status == 'new'" class="relative text-xs">Новая</span>
    ...
    <td class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-500 text-sm leading-5 w-1/5 flex">
      <button @click="showModal = true"
              class="px-4 py-1 border-blue-500 border text-blue-500 rounded transition duration-300 hover:bg-blue-700 hover:text-white focus:outline-none">
        Открыть
      </button>
    </td>
  </tr>
